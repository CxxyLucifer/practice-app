...
class ActionSheetAndroid extends React.Component<Props, any> {
  ...
  render() {
    ...
    const {
      title, message, url, options, destructiveButtonIndex, cancelButtonIndex, excludedActivityTypes, valueSelected
    } = config;
    ...
    const content = share ? (
      ...
      options as Array<string>).map((item, index) => (
        <View key={index} style={[cancelButtonIndex === index ? styles.cancelBtn : null, index == 0 ? { borderBottomWidth: 1, borderBottomColor: '#EEEEEE' } : null]}>
          <TouchableHighlight
            style={[styles.btn]}
            underlayColor={variables.fill_tap}
            onPress={() => this.confirm(index)}
          >
            <Text style={[destructiveButtonIndex === index ? styles.destructiveBtn : null, valueSelected === index ? { color: '#5491DE' } : null]}>
              {item}
            </Text>
          </TouchableHighlight>
